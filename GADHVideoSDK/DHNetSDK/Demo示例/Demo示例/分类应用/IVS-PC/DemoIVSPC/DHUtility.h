...
inline std::string MakeDHPointer(DH_POINT pV)
{
...
inline std::string MakeDHPointers(DH_POINT* pV, int nPointCount)
{
	std::string strRet;
	for (int i = 0; i < nPointCount; i++)
	{
		strRet.append(MakeDHPointer(*(pV + i)));
	}
	return strRet;
}
...
inline std::string MakeDHRect(DH_RECT rectV)
{
	char szText[128] = {0};
