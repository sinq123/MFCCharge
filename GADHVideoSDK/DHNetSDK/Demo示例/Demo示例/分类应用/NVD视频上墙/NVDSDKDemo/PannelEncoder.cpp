...
void CPannelEncoder::OnBtnAddEncoder() 
{
	...
	if (nResponse == IDOK)
	{
		...
		if (pDeviceInfo != NULL)
		{
			...
			pDeviceInfo->nDevChannel = dlg.m_nDevChannel;
...
void CPannelEncoder::OnBtnDelEncoder() 
{
	...
	HTREEITEM selectNode = pParentDlg->m_encoderTree.GetSelectedItem();
	if (selectNode != NULL)
	{
		HTREEITEM parentNode = selectNode;
		DWORD nData = pParentDlg->m_encoderTree.GetItemData(selectNode);
		if (nData <= 32 && nData >= 0)
		{
			parentNode = pParentDlg->m_encoderTree.GetParentItem(selectNode);
			CString strNodeName = pParentDlg->m_encoderTree.GetItemText(selectNode);
			if (strNodeName == ConvertString("Main") || strNodeName == ConvertString("Sub")
				|| strNodeName == ConvertString("Snap"))
			{
				HTREEITEM tempNode = parentNode;
				parentNode = pParentDlg->m_encoderTree.GetParentItem(tempNode);
			}
		}

		DEVICE_INFO *pSelectDevice = (DEVICE_INFO *)pParentDlg->m_encoderTree.GetItemData(parentNode);
...
void CPannelEncoder::OnBtnPlaybackFile() 
{
	...
	if (selectNode != NULL)
	{
		...
		if (nData <= 32 && nData >= 0)
		{
			...
			parentNode = pParentDlg->m_encoderTree.GetParentItem(parentNode);
		}
