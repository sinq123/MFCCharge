...
void CAlarmInputDlg::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, BYTE nChannel, vector<string>& vecAlarmName)
{
	if(pAlarmInfo != NULL)
	{
		m_pstAlarminInfo = new CFG_ALARMIN_INFO[DH_MAX_ALARM_IN_NUM_EX];
		if(NULL == m_pstAlarminInfo)
		{
			return;
		}
		memset(m_pstAlarminInfo, 0, DH_MAX_ALARM_IN_NUM_EX*sizeof(CFG_ALARMIN_INFO));
...
void CAlarmInputDlg::OnSelchangeComboAlarmin() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		for(i = 0; i < m_nAlarmOut; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_ALMOP1+i))->SetCheck((m_pstAlarminInfo[nNum].stuEventHandler.dwAlarmOutMask[0]>>i&0x01)?TRUE:FALSE);
		}
		for(i = 0; i < m_nChannel; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_RECCH1+i))->SetCheck((m_pstAlarminInfo[nNum].stuEventHandler.dwRecordMask[0]>>i&0x01)?TRUE:FALSE);
		}
