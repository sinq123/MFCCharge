...
/*
 *	Function£ºDisplay motion detection alarm information
 *	Input parameter£º
 ...
 *	Note£ºPeriod setup in video loss and camera masking are using the same period setup as motion detection. There is on time table.
							   ...
							   vector<string>& vecAlarmName) 
{
	if(pAlarmInfo != NULL)
	{
		...
		m_nChannel = nChannel;
		m_pstuMotionInfo = new CFG_MOTION_INFO[DH_MAX_VIDEO_IN_NUM_EX];
		if (!m_pstuMotionInfo)
		{
			return;
		}
...
void CAlarmMotion::OnSelchangeComboChannel() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		for(i = 0; i < m_nAlarmOut; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_ALMOP1+i))->SetCheck((m_pstuMotionInfo[nNum].stuEventHandler.dwAlarmOutMask[0]>>i&0x01)?TRUE:FALSE);
		}
		for(i = 0; i < m_nChannel; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_RECCH1+i))->SetCheck((m_pstuMotionInfo[nNum].stuEventHandler.dwRecordMask[0]>>i&0x01)?TRUE:FALSE);
		}
