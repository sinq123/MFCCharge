 ...
 *	nAlarmOut£ºAlarm output channel 
 *	nChannel£ºChannel amount
 */
void CAlarmVLostDlg::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, BYTE nChannel, vector<string>& vecAlarmName)
{
	if(pAlarmInfo != NULL)
	{
		...
		m_nChannel = nChannel;	
		m_pstVideoLostInfo = new CFG_VIDEOLOST_INFO[DH_MAX_VIDEO_IN_NUM_EX];
...
void CAlarmVLostDlg::OnSelchangeComboChannel() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		for(i = 0; i < m_nAlarmOut; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_ALMOP1+i))->SetCheck((m_pstVideoLostInfo[nNum].stuEventHandler.dwAlarmOutMask[0]>>i&0x01)?TRUE:FALSE);
		}
		for(i = 0; i < m_nChannel; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_RECCH1+i))->SetCheck((m_pstVideoLostInfo[nNum].stuEventHandler.dwRecordMask[0]>>i&0x01)?TRUE:FALSE);
		}
		//Time period
