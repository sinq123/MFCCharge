 ...
 */
void CConfigChannel::InitDlgInfo(int nChannelCount, int nVideoStandard, DHDEV_DSP_ENCODECAP &stDspInfo, int nProtoVer, BOOL bNewProtocol)
{
	...
	m_stEncodeInfo = new CFG_ENCODE_INFO[32];
	if(NULL != m_stEncodeInfo)
	{
		memset(m_stEncodeInfo, 0, 32*sizeof(CFG_ENCODE_INFO));
	}
	else
	{
		return;
	}
	m_pstChannelVideoColor = new AV_CFG_ChannelVideoColor[32];
	if(NULL != m_pstChannelVideoColor)
	{
		memset(m_pstChannelVideoColor, 0, 32*sizeof(AV_CFG_ChannelVideoColor));
	}
	else
	{
		return;
	}
	m_pstChannelName = new AV_CFG_ChannelName[32];
	if(NULL != m_pstChannelName)
	{
		memset(m_pstChannelName, 0, 32*sizeof(AV_CFG_ChannelName));
	}
	else
	{
		return;
	...
	OnInitCtrl(nProtoVer, nVideoStandard, nChannelCount);
	//Select channel 0
...
void CConfigChannel::ShowChannelVideoColor(AV_CFG_ChannelVideoColor *pstChannelVideoColor)
{
	...
	for(i = 0 ; i < pstChannelVideoColor->nColorNum; i ++)
	{
		...
		InsertListView(strIndex, strBrightness, strContrast, strSaturation, strChroma, strPlus, 
			...
			strEndHour, strEndMinute, strEndSecond);
	}
}
...
void CConfigChannel::OnSelchangeComboChanno() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		{	
			((CClientDemo5Dlg*)AfxGetMainWnd())->GetOneChannelDspConfig(m_stDspInfo, nIndex, m_stDspInfoEx, m_bShowPix);
...
//P3 ---------------------------------------------------
	...
	if(NULL != pStreamInfo)
	{
		...
		pStreamInfo->stuVideoFormat.emCompression = (CFG_VIDEO_COMPRESSION)m_ctlEncodeType.GetItemData(nIndex);
...
void CConfigChannel::SaveChannelInfo(int nNum)
{
	if(-1 < nNum && -1 < m_nCurStream)
	{
		...
		if(NULL != pInfo)
		{
			...
			memcpy(&m_pstChannelName[nNum].szName, str, AV_CFG_Channel_Name_Len);
		}
	}
}
...
void CConfigChannel::ShowStreamInfo(int nNum, int nItem)
{
	...
	if (m_nProtoVer >= 5)
 	...
 	{
 		m_ctlFrame.SetCurSel(StreamInfo.stuVideoFormat.nFrameRate);
 	}
 	...
 	for(i = 0; i < nItemCount; i++)
 	{
		...
		if(nTagData == StreamInfo.stuVideoFormat.emCompression)
 		{
