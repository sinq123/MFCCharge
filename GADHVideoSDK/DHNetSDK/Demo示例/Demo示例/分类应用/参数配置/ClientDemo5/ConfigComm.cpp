 ...
 */
void CConfigComm::InitDlgInfo(DEV_COMM_CFG_EX *pCommInfo, BYTE nCommCount, BYTE nChannelCount)
{
	if(pCommInfo != NULL)
	{
...
void CConfigComm::OnSelchangeCombo232com() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		m_ctlCommBaud.SetCurSel(m_CommInfo.st232[nNum].struComm.byBaudRate);
...
void CConfigComm::OnSelchangeCombo485com() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		m_ctlBaud485.SetCurSel(m_CommInfo.stDecoder[nNum].struComm.byBaudRate);
...
void CConfigComm::SaveComm232Info(int nNum)
{
	if(-1 < nNum)
	{
		...
		m_CommInfo.st232[nNum].struComm.byBaudRate = m_ctlCommBaud.GetCurSel();
...
void CConfigComm::SaveComm485Info(int nNum)
{
	if(-1 < nNum)
	{
		...
		m_CommInfo.stDecoder[nNum].struComm.byBaudRate = m_ctlBaud485.GetCurSel();
...
BOOL CConfigComm::OnInitDialog() 
{
	...
	int nIndex = m_ctlStopBit485.AddString("1");
	m_ctlCommStopBit.SetItemData(nIndex, 0);
	nIndex = m_ctlStopBit485.AddString("2");
	m_ctlStopBit485.SetItemData(nIndex, 2);
