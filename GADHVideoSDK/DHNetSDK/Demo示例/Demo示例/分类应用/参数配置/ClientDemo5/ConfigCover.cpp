...
void CConfigCover::InitListView()
{
	...
	m_ctlCoverLst.InsertColumn(5, ConvertString("BlendMain"),LVCFMT_LEFT,80,4);
	m_ctlCoverLst.InsertColumn(6, ConvertString("BlendExtra1"),LVCFMT_LEFT,80,5);
	m_ctlCoverLst.InsertColumn(7, ConvertString("BlendExtra2"), LVCFMT_LEFT,80,6);
	m_ctlCoverLst.InsertColumn(8, ConvertString("BlendExtra3"), LVCFMT_LEFT,80,7);
	m_ctlCoverLst.InsertColumn(9, ConvertString("BlendSnapshot"), LVCFMT_LEFT,80,8);
	m_ctlCoverLst.InsertColumn(10, ConvertString("FrontColor-Red"), LVCFMT_LEFT,100,9);
	m_ctlCoverLst.InsertColumn(11, ConvertString("FrontColor-Green"), LVCFMT_LEFT,100,10);
	m_ctlCoverLst.InsertColumn(12, ConvertString("FrontColor-Blue"), LVCFMT_LEFT,100,11);
	m_ctlCoverLst.InsertColumn(13, ConvertString("FrontColor-Alpha"), LVCFMT_LEFT,100,12);
	m_ctlCoverLst.InsertColumn(14, ConvertString("BackColor-Red"),LVCFMT_LEFT,100,13);
	m_ctlCoverLst.InsertColumn(15, ConvertString("BackColor-Green"),LVCFMT_LEFT,100,14);
	m_ctlCoverLst.InsertColumn(16, ConvertString("BackColor-Blue"),LVCFMT_LEFT,100,15);
	m_ctlCoverLst.InsertColumn(17, ConvertString("BackColor-Alpha"),LVCFMT_LEFT,100,16);
}
	...
	if(m_nCurChannel > -1 && m_nCurChannel < 32)    
	{
		if(m_nVideoWidget == 2)
		{
			...
			InsertListView(strID, 
				...
				strBlendMain, strBlendExtra1, strBlendExtra2, strBlendExtra3, strBlendBlendSnapshot,
	...
	if(0 != m_ctlCoverLst.GetItemCount())
	{
		if(0 < m_ctlCoverLst.GetSelectedCount())
		{
			...
			{				
				...
				m_ctlCoverLst.SetItemText(nIndex, 5, strBlendMain);
				m_ctlCoverLst.SetItemText(nIndex, 6, strBlendExtra1);
				m_ctlCoverLst.SetItemText(nIndex, 7, strBlendExtra2);
				m_ctlCoverLst.SetItemText(nIndex, 8, strBlendExtra3);
				m_ctlCoverLst.SetItemText(nIndex, 9, strBlendBlendSnapshot);
				m_ctlCoverLst.SetItemText(nIndex, 10, strFRed);
				m_ctlCoverLst.SetItemText(nIndex, 11, strFGreen);
				m_ctlCoverLst.SetItemText(nIndex, 12, strFBlue);
				m_ctlCoverLst.SetItemText(nIndex, 13, strFAlpha);
				m_ctlCoverLst.SetItemText(nIndex, 14, strBRed);
				m_ctlCoverLst.SetItemText(nIndex, 15, strBGreen);
				m_ctlCoverLst.SetItemText(nIndex, 16, strBBlue);
				m_ctlCoverLst.SetItemText(nIndex, 17, strBAlpha);
			}
		}
	}
}

void CConfigCover::InitDlgInfo(BYTE nLength)
{
	...
	m_pstVideoWidget = new AV_CFG_VideoWidget[32];
	if(NULL == m_pstVideoWidget)
	{
		return;
	}
	memset(m_pstVideoWidget, 0 , sizeof(AV_CFG_VideoWidget) * 32);
	...
	for(BYTE i=0;i<m_nChannelCount;i++)
	{
		str.Format("%d",i+1);
...
void CConfigCover::InsertListView(CString strID, 
		...
		CString strBRed, CString strBGreen, CString strBBlue, CString strBAlpha)
{
	...
	m_ctlCoverLst.SetItemText(nIndex,10,strFRed);
	m_ctlCoverLst.SetItemText(nIndex,11,strFGreen);
	m_ctlCoverLst.SetItemText(nIndex,12,strFBule);
	m_ctlCoverLst.SetItemText(nIndex,13,strFAlpha);
	m_ctlCoverLst.SetItemText(nIndex,14,strBRed);
	m_ctlCoverLst.SetItemText(nIndex,15,strBGreen);
	m_ctlCoverLst.SetItemText(nIndex,16,strBBlue);
...
void CConfigCover::ShowVideoWidgetChannelTitle(AV_CFG_VideoWidgetChannelTitle *pChannelTitleInfo)
{
	...
	//Rect Info
	CString	 strLeft;
	strLeft.Format("%d", pChannelTitleInfo->stuRect.nLeft);
	CString	 strRight;
	strRight.Format("%d", pChannelTitleInfo->stuRect.nRight);
	CString	 strTop;
	strTop.Format("%d", pChannelTitleInfo->stuRect.nTop);
	CString	 strBottom;
	strBottom.Format("%d", pChannelTitleInfo->stuRect.nBottom);

	//Blend Info;
	CString strBlendMain;
	if(pChannelTitleInfo->bEncodeBlend)
	{
	...
	CString strBlendExtra1;
	if(pChannelTitleInfo->bEncodeBlendExtra1)
	{
	...
	CString strBlendExtra2;
	if(pChannelTitleInfo->bEncodeBlendExtra2)
	{
	...
	CString strBlendExtra3;
	if(pChannelTitleInfo->bEncodeBlendExtra3)
	{
	...
	CString strBlendBlendSnapshot;
	if(pChannelTitleInfo->bEncodeBlendSnapshot)
	{
	...
	//Front Color Info
	CString strFRed;
	strFRed.Format("%d", pChannelTitleInfo->stuFrontColor.nRed);
	CString strFGreen;
	strFGreen.Format("%d", pChannelTitleInfo->stuFrontColor.nGreen);
	CString strFBule;
	strFBule.Format("%d", pChannelTitleInfo->stuFrontColor.nBlue);
	CString strFAlpha;
	strFAlpha.Format("%d", pChannelTitleInfo->stuFrontColor.nAlpha);

	//Back Color Info
	CString strBRed;
	strBRed.Format("%d", pChannelTitleInfo->stuBackColor.nRed);
	CString strBGreen;
	strBGreen.Format("%d", pChannelTitleInfo->stuBackColor.nGreen);
	CString strBBlue;
	strBBlue.Format("%d", pChannelTitleInfo->stuBackColor.nBlue);
	CString strBAlpha;
	strBAlpha.Format("%d", pChannelTitleInfo->stuBackColor.nAlpha);
	
	InsertListView(strID, 
		strLeft, strTop, strRight, strBottom, 
		strBlendMain, strBlendExtra1, strBlendExtra2, strBlendExtra3, strBlendBlendSnapshot,
		strFRed, strFGreen, strFBule, strFAlpha,
		strBRed, strBGreen, strBBlue, strBAlpha);
}
void CConfigCover::ShowVideoWidgetTimeTitle(AV_CFG_VideoWidgetTimeTitle *pTimeTitleInfo)
{
	if(NULL == pTimeTitleInfo)
	{
		return;
	}
	...
	//Rect Info
	CString	 strLeft;
	strLeft.Format("%d", pTimeTitleInfo->stuRect.nLeft);
	CString	 strRight;
	strRight.Format("%d", pTimeTitleInfo->stuRect.nRight);
	CString	 strTop;
	strTop.Format("%d", pTimeTitleInfo->stuRect.nTop);
	CString	 strBottom;
	strBottom.Format("%d", pTimeTitleInfo->stuRect.nBottom);
	
	//Blend Info;
	CString strBlendMain;
	if(pTimeTitleInfo->bEncodeBlend)
	{
		strBlendMain = ConvertString("true");
	}
	else
	...
	CString strBlendExtra1;
	if(pTimeTitleInfo->bEncodeBlendExtra1)
	{
		strBlendExtra1 = ConvertString("true");
	}
	else
	...
	CString strBlendExtra2;
	if(pTimeTitleInfo->bEncodeBlendExtra2)
	{
		strBlendExtra2 = ConvertString("true");
	}
	else
	...
	CString strBlendExtra3;
	if(pTimeTitleInfo->bEncodeBlendExtra3)
	{
		strBlendExtra3 = ConvertString("true");
	}
	else
	...
	CString strBlendBlendSnapshot;
	if(pTimeTitleInfo->bEncodeBlendSnapshot)
	{
		strBlendBlendSnapshot = ConvertString("true");
	}
	else
	...
	//Front Color Info
	CString strFRed;
	strFRed.Format("%d", pTimeTitleInfo->stuFrontColor.nRed);
	CString strFGreen;
	strFGreen.Format("%d", pTimeTitleInfo->stuFrontColor.nGreen);
	CString strFBule;
	strFBule.Format("%d", pTimeTitleInfo->stuFrontColor.nBlue);
	CString strFAlpha;
	strFAlpha.Format("%d", pTimeTitleInfo->stuFrontColor.nAlpha);
	
	//Back Color Info
	CString strBRed;
	strBRed.Format("%d", pTimeTitleInfo->stuBackColor.nRed);
	CString strBGreen;
	strBGreen.Format("%d", pTimeTitleInfo->stuBackColor.nGreen);
	CString strBBlue;
	strBBlue.Format("%d", pTimeTitleInfo->stuBackColor.nBlue);
	CString strBAlpha;
	strBAlpha.Format("%d", pTimeTitleInfo->stuBackColor.nAlpha);
...
void CConfigCover::ShowVideoWidgetCover(AV_CFG_VideoWidgetCover *pVideoWidgetCover, int nNum)
{
	...
	{
		AV_CFG_VideoWidgetCover *pInfo = pVideoWidgetCover + i;
		CString strID;
		strID.Format("%d", i+1);

		//Rect Info
		CString	 strLeft;
		strLeft.Format("%d", pInfo->stuRect.nLeft);
		CString	 strRight;
		strRight.Format("%d", pInfo->stuRect.nRight);
		CString	 strTop;
		strTop.Format("%d", pInfo->stuRect.nTop);
		CString	 strBottom;
		strBottom.Format("%d", pInfo->stuRect.nBottom);
		
		//Blend Info;
		CString strBlendMain;
		if(pInfo->bEncodeBlend)
		{
			strBlendMain = ConvertString("true");
		}
		else
		...
		//Front Color Info
		CString strFRed;
		strFRed.Format("%d", pInfo->stuFrontColor.nRed);
		CString strFGreen;
		strFGreen.Format("%d", pInfo->stuFrontColor.nGreen);
		CString strFBule;
		strFBule.Format("%d", pInfo->stuFrontColor.nBlue);
		CString strFAlpha;
		strFAlpha.Format("%d", pInfo->stuFrontColor.nAlpha);
		
		//Back Color Info
		CString strBRed;
		strBRed.Format("%d", pInfo->stuBackColor.nRed);
		CString strBGreen;
		strBGreen.Format("%d", pInfo->stuBackColor.nGreen);
		CString strBBlue;
		strBBlue.Format("%d", pInfo->stuBackColor.nBlue);
		CString strBAlpha;
		strBAlpha.Format("%d", pInfo->stuBackColor.nAlpha);
...
void CConfigCover::SaveVideoWidgetChannelTitle(AV_CFG_VideoWidgetChannelTitle *pChannelTitleInfo)
{
	...
	//Rect Info
	strTmp = m_ctlCoverLst.GetItemText(0, 1);
	pChannelTitleInfo->stuRect.nLeft = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 2);
	pChannelTitleInfo->stuRect.nTop = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 3);
	pChannelTitleInfo->stuRect.nRight = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 4);
	pChannelTitleInfo->stuRect.nBottom = atol(strTmp);
	
	//Blend Info
	strTmp = m_ctlCoverLst.GetItemText(0, 5);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pChannelTitleInfo->bEncodeBlend = 1;
	}
	else
	{
		pChannelTitleInfo->bEncodeBlend = 0;
	}
	
	strTmp = m_ctlCoverLst.GetItemText(0, 6);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pChannelTitleInfo->bEncodeBlendExtra1 = 1;
	}
	else
	{
		pChannelTitleInfo->bEncodeBlendExtra1 = 0;
	}

	strTmp = m_ctlCoverLst.GetItemText(0 ,7);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pChannelTitleInfo->bEncodeBlendExtra2 = 1;
	}
	else
	{
		pChannelTitleInfo->bEncodeBlendExtra2 = 0;
	}

	strTmp = m_ctlCoverLst.GetItemText(0, 8);
	if(strTmp == "true" || strTmp  == ConvertString("true"))
	{
		pChannelTitleInfo->bEncodeBlendExtra3 = 1;
	}
	else
	{
		pChannelTitleInfo->bEncodeBlendExtra3 = 0;
	}

	strTmp = m_ctlCoverLst.GetItemText(0, 9);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pChannelTitleInfo->bEncodeBlendSnapshot = 1;
	}
	else
	{
		pChannelTitleInfo->bEncodeBlendSnapshot = 0;
	}

	// Front color
	strTmp = m_ctlCoverLst.GetItemText(0, 10);
	pChannelTitleInfo->stuFrontColor.nRed = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 11);
	pChannelTitleInfo->stuFrontColor.nGreen = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 12);
	pChannelTitleInfo->stuFrontColor.nBlue = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 13);
	pChannelTitleInfo->stuFrontColor.nAlpha = atol(strTmp);

	// Back  color
	strTmp = m_ctlCoverLst.GetItemText(0, 14);
	pChannelTitleInfo->stuBackColor.nRed = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 15);
	pChannelTitleInfo->stuBackColor.nGreen = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 16);
	pChannelTitleInfo->stuBackColor.nBlue = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 17);
	pChannelTitleInfo->stuBackColor.nAlpha = atol(strTmp);
...
void CConfigCover::SaveVideoWidgetTimeTitle(AV_CFG_VideoWidgetTimeTitle *pTimeTitleInfo)
{
	...
	//Rect Info
	strTmp = m_ctlCoverLst.GetItemText(0, 1);
	pTimeTitleInfo->stuRect.nLeft = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 2);
	pTimeTitleInfo->stuRect.nTop = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 3);
	pTimeTitleInfo->stuRect.nRight = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 4);
	pTimeTitleInfo->stuRect.nBottom = atol(strTmp);
	
	//Blend Info
	strTmp = m_ctlCoverLst.GetItemText(0, 5);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pTimeTitleInfo->bEncodeBlend = 1;
	}
	else
	{
		pTimeTitleInfo->bEncodeBlend = 0;
	}
	
	strTmp = m_ctlCoverLst.GetItemText(0, 6);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pTimeTitleInfo->bEncodeBlendExtra1 = 1;
	}
	else
	{
		pTimeTitleInfo->bEncodeBlendExtra1 = 0;
	}
	
	strTmp = m_ctlCoverLst.GetItemText(0 ,7);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pTimeTitleInfo->bEncodeBlendExtra2 = 1;
	}
	else
	{
		pTimeTitleInfo->bEncodeBlendExtra2 = 0;
	}
	
	strTmp = m_ctlCoverLst.GetItemText(0, 8);
	if(strTmp == "true" || strTmp  == ConvertString("true"))
	{
		pTimeTitleInfo->bEncodeBlendExtra3 = 1;
	}
	else
	{
		pTimeTitleInfo->bEncodeBlendExtra3 = 0;
	}
	
	strTmp = m_ctlCoverLst.GetItemText(0, 9);
	if(strTmp == "true" || strTmp == ConvertString("true"))
	{
		pTimeTitleInfo->bEncodeBlendSnapshot = 1;
	}
	else
	{
		pTimeTitleInfo->bEncodeBlendSnapshot = 0;
	}
	
	// Front color
	strTmp = m_ctlCoverLst.GetItemText(0, 10);
	pTimeTitleInfo->stuFrontColor.nRed = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 11);
	pTimeTitleInfo->stuFrontColor.nGreen = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 12);
	pTimeTitleInfo->stuFrontColor.nBlue = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 13);
	pTimeTitleInfo->stuFrontColor.nAlpha = atol(strTmp);
	
	// Back  color
	strTmp = m_ctlCoverLst.GetItemText(0, 14);
	pTimeTitleInfo->stuBackColor.nRed = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 15);
	pTimeTitleInfo->stuBackColor.nGreen = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 16);
	pTimeTitleInfo->stuBackColor.nBlue = atol(strTmp);
	strTmp = m_ctlCoverLst.GetItemText(0, 17);
	pTimeTitleInfo->stuBackColor.nAlpha = atol(strTmp);
}
void CConfigCover::SaveVideoWidgetCover(AV_CFG_VideoWidgetCover *pVideoWidgetCover, int *nNum)
{
	if(NULL == pVideoWidgetCover || NULL == nNum)
	{
		return ;
	}
	...
	*nNum = nCount;
	...
	for(i = 0; i < nCount; i++)
	{
		...
		pInfo->stuRect.nLeft = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 2);
		pInfo->stuRect.nTop = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 3);
		pInfo->stuRect.nRight = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 4);
		pInfo->stuRect.nBottom = atol(strTmp);
		
		//Blend Info
		strTmp = m_ctlCoverLst.GetItemText(i, 5);
		if(strTmp == "true" || strTmp == ConvertString("true"))
		{
			pInfo->bEncodeBlend = 1;
		}
		else
		{
			pInfo->bEncodeBlend = 0;
		}
		
		strTmp = m_ctlCoverLst.GetItemText(i, 6);
		if(strTmp == "true" || strTmp == ConvertString("true"))
		{
			pInfo->bEncodeBlendExtra1 = 1;
		}
		else
		{
			pInfo->bEncodeBlendExtra1 = 0;
		}
		
		strTmp = m_ctlCoverLst.GetItemText(i ,7);
		if(strTmp == "true" || strTmp == ConvertString("true"))
		{
			pInfo->bEncodeBlendExtra2 = 1;
		}
		else
		{
			pInfo->bEncodeBlendExtra2 = 0;
		}
		
		strTmp = m_ctlCoverLst.GetItemText(i, 8);
		if(strTmp == "true" || strTmp  == ConvertString("true"))
		{
			pInfo->bEncodeBlendExtra3 = 1;
		}
		else
		{
			pInfo->bEncodeBlendExtra3 = 0;
		}
		
		strTmp = m_ctlCoverLst.GetItemText(i, 9);
		if(strTmp == "true" || strTmp == ConvertString("true"))
		{
			pInfo->bEncodeBlendSnapshot = 1;
		}
		else
		{
			pInfo->bEncodeBlendSnapshot = 0;
		}
		
		// Front color
		strTmp = m_ctlCoverLst.GetItemText(i, 10);
		pInfo->stuFrontColor.nRed = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 11);
		pInfo->stuFrontColor.nGreen = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 12);
		pInfo->stuFrontColor.nBlue = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 13);
		pInfo->stuFrontColor.nAlpha = atol(strTmp);
		
		// Back  color
		strTmp = m_ctlCoverLst.GetItemText(i, 14);
		pInfo->stuBackColor.nRed = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 15);
		pInfo->stuBackColor.nGreen = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 16);
		pInfo->stuBackColor.nBlue = atol(strTmp);
		strTmp = m_ctlCoverLst.GetItemText(i, 17);
		pInfo->stuBackColor.nAlpha = atol(strTmp);
	}
}
