		...
		{	
			...
			if(bValid)
			{
				if (m_bNewProtocol == FALSE)
				{
					...
					((CClientDemo5Dlg*)AfxGetMainWnd())->SetRecordConfigInfo(m_RecordInfo,m_nChannelCount);
				}
				else
				{
					((CClientDemo5Dlg*)AfxGetMainWnd())->SetOneRecordConfigInfo(&m_RecordInfo[m_nCurChannel], m_nCurChannel);
				}		
			}
		}
	}
 ...
 */
void CConfigRecord::InitDlgInfo(DHDEV_RECORD_CFG *pRecordInfo, BYTE nChannelCount, int nProtoVer, BOOL bNewProtocol)
{
...
void CConfigRecord::OnSelchangeComboChanno() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		{
			((CClientDemo5Dlg*)AfxGetMainWnd())->GetOneRecordConfigInfo(&m_RecordInfo[nNum], nNum);
		}

		//Display channel corresponding information 
		SetDlgItemInt(IDC_EDIT_PRERECLEN, m_RecordInfo[nNum].byPreRecordLen);
		m_ctlRedundancy.SetCheck((m_RecordInfo[nNum].byRedundancyEn)?1:0);
		//Week 
		...
		m_ctlRecordType.SetCurSel(m_RecordInfo[nNum].byRecordType);
...
BOOL CConfigRecord::SaveWeekInfo(int nNum, int nWeek)
{
	...
	if(-1 < nNum && -1 < nWeek)
	{
		if(UpdateData(TRUE))
		{
			DH_TSECT *pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][0]);
			...
			pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][1]);
			...
			pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][2]);
			...
			pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][3]);
			...
			pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][4]);
			...
			pPeriodInfo = &(m_RecordInfo[nNum].stSect[nWeek][5]);
