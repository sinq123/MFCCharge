...
void  CConfigVideoIn::InitDlgInfo(int nChannelCount)
{
	...
	m_pstCapVideoInput = new CFG_CAP_VIDEOINPUT_INFO[DH_MAX_VIDEO_IN_NUM_EX];
	...
	{
		memset(m_pstCapVideoInput, 0 ,DH_MAX_VIDEO_IN_NUM_EX*sizeof(CFG_CAP_VIDEOINPUT_INFO));
	}

	m_pstVideoInOption = new CFG_VIDEO_IN_OPTIONS[DH_MAX_VIDEO_IN_NUM_EX];
	...
	{
		memset(m_pstVideoInOption, 0 ,DH_MAX_VIDEO_IN_NUM_EX*sizeof(CFG_VIDEO_IN_OPTIONS));
	}
...
BOOL CConfigVideoIn::SaveOptionInfo(int nNum,int nItem)
{
	...
	IrisAuto  = m_ctlMirror.GetCheck();
	mirror    = m_ctlIrisAuto.GetCheck();
