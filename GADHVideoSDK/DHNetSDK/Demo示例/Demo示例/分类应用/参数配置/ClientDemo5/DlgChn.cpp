...
BOOL CDlgChn::SetChnState(BYTE* bState, int nLen /* = 32 */)
{
	...
	for(i=0; i<32; i++)
	{
		GetDlgItem(IDC_CHECK_CHN0 + i)->ShowWindow(SW_SHOW);
	}
	for(i=nLen; i<32; i++)
	{
		GetDlgItem(IDC_CHECK_CHN0 + i)->ShowWindow(SW_HIDE);
	}
	
	UINT uIDBase = IDC_CHECK_CHN0;
	BOOL bCheck = FALSE;
