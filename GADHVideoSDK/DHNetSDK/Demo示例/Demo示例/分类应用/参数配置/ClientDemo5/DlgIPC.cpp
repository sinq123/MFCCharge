...
void CDlgIPC::GetIPFilterCfg()
{
	...
	//TODO
	m_bList = m_stuIPFilterCfg.dwEnable;
	...
	for(int i = 0; i < m_stuIPFilterCfg.TrustIP.dwIPNum; i++)
	{
		m_listWhite.AddString(m_stuIPFilterCfg.TrustIP.SZIP[i]);
	}
	for(int j = 0; j < m_stuIPFilterCfg.BannedIP.dwIPNum; j++)
	{
		m_listBlack.AddString(m_stuIPFilterCfg.BannedIP.SZIP[j]);
	}
...
void CDlgIPC::SetIPFilterCfg()
{
	...
	m_stuIPFilterCfg.dwEnable = m_bList;
	m_stuIPFilterCfg.dwType = m_bWhite == TRUE ? 0 : 1;//0,白名单，1黑名单
	m_stuIPFilterCfg.TrustIP.dwIPNum = m_listWhite.GetCount();
	m_stuIPFilterCfg.BannedIP.dwIPNum = m_listBlack.GetCount();
	//black and white list
	CString strWhite;
	CString strBlack;
	for(int i = 0; i < m_stuIPFilterCfg.TrustIP.dwIPNum; i++)
	{
		m_listWhite.GetText(i, strWhite);
		strcpy(m_stuIPFilterCfg.TrustIP.SZIP[i], strWhite.GetBuffer(0));
	}
	for(int j = 0; j < m_stuIPFilterCfg.BannedIP.dwIPNum; j++)
	{
		m_listBlack.GetText(j, strBlack);
