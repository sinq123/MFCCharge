...
#define DH_WAIT_TIMEOUT	  7000		// 等待消息允许最大时间
/////////////////////////////////////////////////////////////////////////////
...
BOOL CClientDemo6Dlg::OnInitDialog()
{
	...
	m_ctlDvrIP.SetAddress(20,2,4,129);
	InitListView();
...
void CClientDemo6Dlg::OnBtnLogin() 
{
	...
	if(bValid)
	{
		...
		else
		{
			...
			int nRetLen = 0;
			BOOL bRet = CLIENT_QuerySystemInfo(m_LoginID, ABILITY_DEVALL_INFO, (char*)&m_stuEnable, sizeof(DH_DEV_ENABLE_INFO), &nRetLen);
...
void CClientDemo6Dlg::OnButtonQuery() 
{
	...
	if(0 != m_LoginID)
	{
		...
		else
		{
			...
			int nRetLogNum = 0;
			BOOL bRet = CLIENT_QueryDeviceLog(m_LoginID, &stuLogParam, (char*)szLogInfos, 1024 * sizeof(DH_DEVICE_LOG_ITEM_EX), &nRetLogNum, DH_WAIT_TIMEOUT);
			if (bRet)
			{
				//display log info
				if (nRetLogNum <= 0)
				...
				else if (nRetLogNum < m_nPageLine)
				{
					...
					do 
					{
						...
						bRet = CLIENT_QueryDeviceLog(m_LoginID, &stuLogParam, (char*)szLogInfos, 1024 * sizeof(DH_DEVICE_LOG_ITEM_EX), &nRetLogNum, DH_WAIT_TIMEOUT);
						if (bRet)
						{
							// no more
							if (nRetLogNum <= 0)
...
void CClientDemo6Dlg::TransLogInfo(void *pLoginfo, int nStructType, CString &strLogType, CString &strLogTime, CString &strLogContext)
{
	if (pLoginfo == NULL)
	{
		return;
...
void CClientDemo6Dlg::DisplayQueryResult(char *recBuf, int nLength)
{
	...
	for(int i=0;i<nCount;i++)
	{
		...
		TransLogInfo(pLogItem, 1, strLogType, strLogTime, strLogContext);
...
BOOL CClientDemo6Dlg::JudgeType(int nQueryType, int nInfoType)
{
	...
	case DHLOG_SYSTEM:
		if(nInfoType == DH_LOG_REBOOT || nInfoType == DH_LOG_SHUT || nInfoType == DH_LOG_UPGRADE 
			|| nInfoType == DH_LOG_UPGRADE || nInfoType == DH_LOG_SYSTIME_UPDATE)
		{
			bShow = TRUE;
		}
		break;
	...
	case DHLOG_STORAGE:
		if(nInfoType == DH_LOG_FSERROR || nInfoType == DH_LOG_HDD_WERR || nInfoType == DH_LOG_HDD_RERR
			...
			|| nInfoType == DH_LOG_SDPLUG_OUT || nInfoType == DH_LOG_HDD_START_INFO || nInfoType == DH_LOG_HDD_SLIGHT_ERR || nInfoType == DH_LOG_HDD_SERIOUS_ERR || nInfoType == DH_LOG_HDD_WORKING_DISK || nInfoType == DH_LOG_HDD_OTHER_ERROR)
		{
			bShow = TRUE;
		}
		break;
	case DHLOG_ALARM:
		if(nInfoType == DH_LOG_ALM_IN || nInfoType == DH_LOG_NETALM_IN || nInfoType == DH_LOG_ALM_END
			...
			|| nInfoType == DH_LOG_NET_PORT_BIND_FAILED || nInfoType == DH_LOG_INTELLI_ALARM_PLUSE
			|| nInfoType == DH_LOG_INTELLI_ALARM_IN || nInfoType == DH_LOG_INTELLI_ALARM_END || nInfoType == DH_LOG_HDD_NOSPACE)
		{
			bShow = TRUE;
		}
		break;
...
CString CClientDemo6Dlg::ConvertCfgType(const DH_LOG_ITEM* pLogItem)
{
	...
	case 43:
		...
		break;
	case  100:
		strCfgType.Format(ConvertString("IP Modify"), pLogItem->context[0], pLogItem->context[1], pLogItem->context[2], pLogItem->context[3]);
		break;
	case  101:
		strCfgType.Format(ConvertString("Gateway Modify"), pLogItem->context[0], pLogItem->context[1], pLogItem->context[2], pLogItem->context[3]);
		break;
	case  102:
		strCfgType.Format(ConvertString("Subnetwork Modify"), pLogItem->context[0], pLogItem->context[1], pLogItem->context[2], pLogItem->context[3]);
		break;
	case 123:
...
void CClientDemo6Dlg::DisPlayNewLogInfo(DH_DEVICE_LOG_ITEM_EX* pLogItem, int nCount, BOOL bAppend/* = FALSE*/)
{
	...
	if(pLogItem->bUnionType == 0)//新日志
	{
		for(int i=0; i< nCount; i++)
		{
			TransLogInfo(&pLogItem[i], 0, strLogType, strLogTime, strLogContext);
	...
	else if(pLogItem->bUnionType == 1)//旧日志
	{
		for(int i=0; i < nCount; i++)
		{
			...
			TransLogInfo(pOldLogItem, 1, strLogType, strLogTime, strLogContext);
