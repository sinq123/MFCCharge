...
void CDialogEventRule::ReceiveRealData(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize)
{
...
void CDialogEventRule::OnButtonStartPreview() 
{
	...
	//Enable stream
	BOOL bOpenRet = g_PlayAPI.PLAY_OpenStream(nPort,0,0,1024*100);
	if(bOpenRet)
	{
...
/* SDK回调函数                                                          */
/************************************************************************/
int  CDialogEventRule::RealLoadPicCallback (LONG lAnalyzerHandle, DWORD dwAlarmType, void* pAlarmInfo, BYTE *pBuffer, DWORD dwBufSize, DWORD dwUser, int nSequence, void *userdata)
{
...
// 	}
	...
	for (int nChn = 0; nChn < m_netDevInfo.byChanNum; nChn++)
	{
		...
		{
			LONG lRet = CLIENT_RealLoadPictureEx(m_lLoginID, nChn, EVENT_IVS_ALL, bNeedData, CDialogEventRule::RealLoadPicCallback, (DWORD)this, (void*)&stReserved);
			if (lRet == 0)
			{
...
//多次订阅多种事件
			for (i = 0; i < nAlarmTypeCount; i++)
			{
				...
				LONG lRet = CLIENT_RealLoadPictureEx(m_lLoginID, nChn, dwAlarmType, bNeedData,CDialogEventRule::RealLoadPicCallback, (DWORD)this, (void*)&stReserved);
				if (lRet == 0)
				{
					MessageBox(ConvertString("Subscribe picture failed!"), ConvertString("Prompt"));
					continue;
...
void CDialogEventRule::OnButtonManualSnap() 
{
	...
	memcpy(stuSanpParam.bySequence, m_strSnapSeq, m_strSnapSeq.GetLength());
...
void CDialogEventRule::OnDisConnect(LONG lLoginID, char *pchDVRIP, LONG nDVRPort, DWORD dwUser)
{
	OnButtonStop();
	OnButtonStart();
}
