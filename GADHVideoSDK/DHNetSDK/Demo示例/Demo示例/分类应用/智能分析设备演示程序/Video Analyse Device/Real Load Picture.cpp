...
BOOL TryDisplayImageAndLPPart(const char* szJPGPath, const ALARM_ITEM* item, CWnd* fullImageWindow, CWnd* lpImageWindow)
{
	...
	if (IsTypeHasLP(item->nAlarmType))//判断事件内容是否包含车牌
	{
		...
		DH_RECT* dhRect = &(trafficGate->stuObject.BoundingBox);
		...
		dstRect.left	= ceil((double)(nWidth  * dhRect->left)   / 8192);
		dstRect.right	= ceil((double)(nWidth  * dhRect->right)  / 8192);
		dstRect.bottom	= ceil((double)(nHeight * dhRect->bottom) / 8192);
		dstRect.top		= ceil((double)(nHeight * dhRect->top)    / 8192);
...
void GetVehicleInfo(int  nGetWhat, DH_MSG_OBJECT *pObject, std::vector<ColDes>* vTitles, std::vector<std::string>* vContent)
{
	...
	if (nGetWhat & GET_SI_CONTENT && pObject != NULL)
	{
		...
		vContent->push_back(MakeDHRect(pObject->BoundingBox));
		vContent->push_back(MakeDHPointer(pObject->Center));
		vContent->push_back(I2Str(pObject->nPolygonNum));
		vContent->push_back(MakeDHPointers(pObject->Contour, pObject->nPolygonNum));
	...
	case EVENT_IVS_TRAFFICACCIDENT:
		{
			...
			if (nGetWhat & GET_SI_CONTENT && pInfo != NULL)
			{
				...
				for (int i = 0; i < pInfo->nObjectNum; i ++)
				{
					...
					vContent->push_back(MakeDHRect(pInfo->stuObjectIDs[i].BoundingBox));
					vContent->push_back(MakeDHPointer(pInfo->stuObjectIDs[i].Center));
					vContent->push_back(I2Str(pInfo->stuObjectIDs[i].nPolygonNum));
					vContent->push_back(MakeDHPointers(pInfo->stuObjectIDs[i].Contour, pInfo->stuObjectIDs[i].nPolygonNum));
	...
	case EVENT_IVS_TRAFFICGATE:
		{
			...
			if (nGetWhat & GET_SI_CONTENT && pInfo != NULL)
			{
				...
				vContent->push_back(MakeDHRect(pInfo->stuVehicle.BoundingBox));
				vContent->push_back(MakeDHPointer(pInfo->stuVehicle.Center));
				vContent->push_back(I2Str(pInfo->stuVehicle.nPolygonNum));
				vContent->push_back(MakeDHPointers(pInfo->stuVehicle.Contour, pInfo->stuObject.nPolygonNum));
