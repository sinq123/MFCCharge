...
void CDemoMonitorWallDlg::OnDblclkTreeDevice(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	{		
		...
		// 大屏控制器特殊处理
// 		if (stricmp(pData->pDevice->GetDeviceClass(), CEncoderDSCON::DCID()) == 0)
// 		{
//			TreeItemDataDscon* pMatrixData = (TreeItemDataDscon*)pData;
// 			if (pMatrixData->nInputType == 0)
// 			{
// 				sprintf(stuSrc.szDeviceID, "Local/Matrix/%s", pMatrixData->szDevID);
// 			}
// 			else
// 			{
//				strcpy(stuSrc.szDeviceID, pMatrixData->szDevID);
//			}
//		}
	...
	if (LLONG lLoginID = CLIENT_Login((char*)m_pDecoder->GetIP(), m_pDecoder->GetPort(), (char*)m_pDecoder->GetUser(), (char*)m_pDecoder->GetPwd(), &stuDevInfo, &nError))
	{
		...
		if (pDev)
		{
			if (pDev->Update() && pDev->GetEncChnMap().size() > 0)
			{
				...
				if (stricmp(pDev->GetDeviceClass(), CEncoderNVR::DCID()) == 0
					|| stricmp(pDev->GetDeviceClass(), CEncoderDSCON::DCID()) == 0)
				{
