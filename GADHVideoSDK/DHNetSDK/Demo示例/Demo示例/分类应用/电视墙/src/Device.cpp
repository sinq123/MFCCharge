...
LPCTSTR CEncChannel::ToString() const
{
	...
	{
		strName.Format("Chn%d(Card%d_%d)", m_nChannelID, m_nCardID, m_nCardSlot);
	}
	else
	{
		strName.Format("Chn%d", m_nChannelID);
	}
	return strName;
}
...
BOOL CDecoder::UpdateCompositeChannel()
{
	...
	// 取融合屏虚拟通道信息
	DH_COMPOSITE_CHANNEL* pstuComposite = new DH_COMPOSITE_CHANNEL[m_stuProdDef.nVideoOutputCompositeChannels];
	ZeroMemory(pstuComposite, sizeof(DH_COMPOSITE_CHANNEL) * m_stuProdDef.nVideoOutputCompositeChannels);
	for (int i = 0; i < m_stuProdDef.nVideoOutputCompositeChannels; ++i)
	{
	...
	if (CLIENT_QueryDevState(m_lLoginID, DH_DEVSTATE_COMPOSITE_CHN, 
		(char*)pstuComposite, pstuComposite->dwSize * m_stuProdDef.nVideoOutputCompositeChannels, &nRetLen))
	{
...
BOOL CEncoder::UpdateNormalChannel()
{
	...
	DWORD dwRet = 0;
	DHDEV_SYSTEM_ATTR_CFG stuCfg = {0};
	if (CLIENT_GetDevConfig(m_lLoginID, DH_DEV_DEVICECFG, -1, &stuCfg, sizeof(stuCfg), &dwRet, 3000))
	{
		for (int i = 0; i < stuCfg.byVideoCaptureNum; ++i)
		{
