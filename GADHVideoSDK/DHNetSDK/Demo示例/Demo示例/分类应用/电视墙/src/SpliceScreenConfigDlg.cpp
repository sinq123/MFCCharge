...
void CSpliceScreenConfigDlg::InitUI()
{
	...
	else
	{
		...
		{
			AV_CFG_MonitorWall stuWall = { sizeof(AV_CFG_MonitorWall) };
			for (int i = 0; i < AV_CFG_Max_Block_In_Wall; ++i)
			{
				stuWall.stuBlocks[i].nStructSize = sizeof(AV_CFG_MonitorWallBlock);				
				stuWall.stuBlocks[i].stuRect.nStructSize = sizeof(AV_CFG_MonitorWallTVOut);	
				for (int j = 0; j < AV_CFG_Max_TV_In_Block; ++j)
				{
					stuWall.stuBlocks[i].stuTVs[j].nStructSize = sizeof(AV_CFG_MonitorWallTVOut);
				}
			}
			
			int nRetLen = 0;
			if (CLIENT_ParseData(CFG_CMD_MONITORWALL, pszJsonBuf, &stuWall, sizeof(stuWall), &nRetLen))
			{
				m_wndWall.ClearBlock();
				m_wndWall.SetName(stuWall.szName);
				m_wndWall.SetGridLayout(CSize(stuWall.nColumn, stuWall.nLine));
				for (int i = 0; i < stuWall.nBlockCount; ++i)
				{
					AV_CFG_MonitorWallBlock* pBlockInfo = stuWall.stuBlocks + i;
