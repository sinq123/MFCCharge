...
void CClientDemo1Dlg::DeviceDisConnect(LONG lLoginID, char *sDVRIP, LONG nDVRPort)
{
...
BOOL CClientDemo1Dlg::ReceiveMessage(LONG lLoginID, LONG lCommand, char *pchDVRIP, LONG nDVRPort, char *pBuf, DWORD dwBufLen)
{
	BOOL bRet = FALSE;
...
BOOL CClientDemo1Dlg::ReceiveMessageEx(LLONG lLoginID, LONG lCommand, char *pchDVRIP, LONG nDVRPort, char *pBuf, DWORD dwBufLen)
{
	...
	if(bIsTimeOut)
	{
		...
		case DH_DISKFULL_ALARM_EX:		//0x2106	//HDD full alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_DISKFULL_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm,pBuf,min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
			break;
		case DH_DISKERROR_ALARM_EX:		//0x2107	//HDD malfunction alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_DISKERROR_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm,pBuf,min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
			break;
		case DH_ENCODER_ALARM_EX:		//	0x210A	//Encoder alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_ENCODER_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm,pBuf,min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
			break;
		case DH_URGENCY_ALARM_EX:		//	0x210B	//Emergency alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_URGENCY_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm,pBuf,min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
			break;
		case DH_WIRELESS_ALARM_EX:		//0x210C	//Wireless alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_WIRELESS_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm,pBuf,min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
			break;
		case DH_ALARM_DECODER_ALARM_EX: //0x210E	//Alarm decoder alarm 
			{
				alarmAlarm.m_AlarmCommand = DH_ALARM_DECODER_ALARMEX;
		...
		case DH_STATIC_ALARM_EX:
			{
				alarmAlarm.m_AlarmCommand = DH_STATIC_ALARMEX;
				memcpy(&alarmAlarm.m_dwAlarm, pBuf, min(dwBufLen, sizeof(alarmAlarm.m_dwAlarm)));
				bRet = TRUE;
			}
		case DH_ALARM_ARM_DISARM_STATE:
			{
				alarmAlarm.m_AlarmCommand = DH_ALARM_ARM_DISARMSTATE;
...
void CClientDemo1Dlg::DHCommAlarm(char *pBuf, DWORD dwBufLen)
{
	...
	alarmInfo.m_AlarmCommand = DH_COMMON_ALARM;
	int i = 0;
	double x = 2;
	//External alarm 
...
void CClientDemo1Dlg::DHShelterAlarm(char *pBuf, DWORD dwBufLen)
{
	if (dwBufLen != 16)
	{
		return;
	...
	CAlarmInfo alarmInfo;
	double x = 2;
	alarmInfo.m_AlarmCommand = DH_VIDEOSHELTER_ALARM;
	for(int i=0;i<16;i++)
	{
		char c = pBuf[i];
		if(1 == c)
...
//HDD full alarm 
void CClientDemo1Dlg::DHDiskFullAlarm(char *pBuf, DWORD dwBufLen)
{
	...
	CAlarmInfo alarmInfo;
	alarmInfo.m_AlarmCommand = DH_DISKFULL_ALARM;
...
void CClientDemo1Dlg::DHDiskError(char *pBuf, DWORD dwBufLen)
{
	if (dwBufLen != sizeof(DWORD))
	{
		return;
	...
	CAlarmInfo alarmInfo;
	alarmInfo.m_AlarmCommand = DH_DISKERROR_ALARM;
	alarmInfo.m_DiskError = *(DWORD*)pBuf;
	m_listAlarmInfo.AddTail(alarmInfo);
}

//Video detection alarm 
void CClientDemo1Dlg::DHSoundAlarm(char *pBuf, DWORD dwBufLen)
{
	if (dwBufLen != 16)
	{
		return;
	...
	CAlarmInfo alarmInfo;	
	double x = 2;
	alarmInfo.m_AlarmCommand = DH_SOUNDDETECT_ALARM;
	for(int i=0;i<16;i++)
	{
		char c = pBuf[i];
		if(1 == c)
		...
		if(bIsTimeOut)
		{
			if(!(m_listAlarmInfo.IsEmpty()))
			{
				...
				for(int i=0;i<m_listAlarmInfo.GetCount();i++)
				{
					...
					switch(enAlarmType) {
					case DH_COMMON_ALARM:		//General alarm 
						...
						//TRACE("DH_COMMON_ALARM.wInputAlarm=%d  wVideoLost=%d  wMotionAlarm=%d\n",wInputAlarm,wVideoLost,wMotionAlarm);
						//Display alarm 
						...
						break;
					case DH_VIDEOSHELTER_ALARM:		//Camera masking alarm 
						...
						break;
					case DH_SOUNDDETECT_ALARM:		//Audio alarm 
						...
						break;
					case DH_DISKFULL_ALARM:			//Alarm when HDD capacity is insufficient
						...
						break;
					case DH_DISKERROR_ALARM:		//HDD malfunction alarm 
