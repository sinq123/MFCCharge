...
BOOL CClientDemo3Dlg::OnInitDialog()
{
	...
	//Device default IP
	m_ctlDvrIP.SetAddress(10, 42, 1, 150);
	InitTabControl();
	InitNetSDK();
...
void CClientDemo3Dlg::OnTimer(UINT nIDEvent) 
{
	...
	if(nIDEvent == m_nTimer)
	{
		...
		//	DH_PlayBackMode ePlayMode = (DH_PlayBackMode)(m_pTabTimeDlg->m_ctlPlayMode.GetItemData(nIndex));
...
int Compare( const NET_TIME *pSrcTime, const NET_TIME *pDestTime)
{
	...
	srcTime.wYear     =    pSrcTime->dwYear;
	srcTime.wMonth  =    pSrcTime->dwMonth;
	srcTime.wDay       =   pSrcTime->dwDay;
	srcTime.wHour     =   pSrcTime->dwHour;
	srcTime.wMinute   =  pSrcTime->dwMinute;
	srcTime.wSecond  =   pSrcTime->dwSecond;
    /// dest
	destTime.wYear     =    pDestTime->dwYear;
	destTime.wMonth  =    pDestTime->dwMonth;
	destTime.wDay       =   pDestTime->dwDay;
	destTime.wHour     =   pDestTime->dwHour;
	destTime.wMinute   =  pDestTime->dwMinute;
	destTime.wSecond  =   pDestTime->dwSecond;
...
void CClientDemo3Dlg::AddTimeToDateTimeCtl(NET_RECORDFILE_INFO fileInfo[2000], int nFileCount)
{
	...
	for (int i = 0;i < nFileCount;i++)
	{
		...
		if (fileInfo[i].endtime.dwDay == fileInfo[i].starttime.dwDay + 1)
		{
			lEndTime = 24*3600;
		}
		else
...
//	m_DayTimePickSdk.AddTimeBlock(0, 60*60*24, RGB(0,0,0));
	...
	if (fileInfo.endtime.dwDay > fileInfo.starttime.dwDay)
	{
