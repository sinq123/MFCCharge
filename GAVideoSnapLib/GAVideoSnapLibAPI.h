...
class AFX_EXT_CLASS CGAVideoSnapLibAPI
{
public:
	static CGAVideoSnapLibAPI& GetInstance(void);

private:
	CGAVideoSnapLibAPI(void);
	virtual ~CGAVideoSnapLibAPI(void);
	CGAVideoSnapLibAPI(const CGAVideoSnapLibAPI &);
	CGAVideoSnapLibAPI & operator =(const CGAVideoSnapLibAPI &);
	friend class std::auto_ptr<CGAVideoSnapLibAPI>;
...
public:
	...
	bool TakeStartDimensionLEDVideo(const CStringW& strRunningNumber, const COleDateTime& odtVideoStartTime, const CStringW& strLineNumber);
	bool TakeEndDimensionLEDVideo(const CStringW& strRunningNumber, const COleDateTime& odtVideoEndTime, const CStringW& strLineNumber);
	// 整备
