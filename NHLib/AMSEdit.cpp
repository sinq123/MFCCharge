...
CAMSEdit::MaskedBehavior::MaskedBehavior(CAMSEdit* pEdit, const CString& strMask /*= _T("")*/) :
	...
	m_strMask(strMask)
{
	m_arraySymbols.Add(Symbol(_T('#'), _istdigit));  // default mask symbol
}
	...
	m_cSep('/')
{
	...
	m_uFlags |= OnKillFocus_SetValid_IfInvalid;	//检测到日期无效时，自动设置为有效值。
}
...
void CAMSEdit::DateBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	...
	switch (nStart)
	{
		...
		case 7:		// SECOND SLASH	(year's first digit)
		{
			int nSlash = 0;
			if (c == m_cSep)
				nSlash = 1;
		...
		case 3:
		{
			if (IsValidYearDigit(c, nStart - GetYearStartPosition()) && _ShouldEnter(c))
			{
				if (nLen > nStart)
				{
					...
					for (; nStart + 1 < nLen && nStart < GetYearStartPosition()+3; nStart++)
					{
						if (!IsValidYearDigit(strText[nStart + 1], nStart - (GetYearStartPosition() - 1)))
						{
							m_pEdit->SetSel(nStart + 1, GetYearStartPosition()+4, FALSE);
							CString strPortion;
							for (int iPos = nStart + 1; iPos < nLen && iPos < GetYearStartPosition()+4; iPos++)
...
{
	return ((m_uFlags & DayBeforeMonth) ? 8 : 5);
}
...
{
	return ((m_uFlags & DayBeforeMonth) ? 5 : 8);
}
...
CString CAMSEdit::DateBehavior::GetFormattedDate(int nYear, int nMonth, int nDay) const
{
	...
	strText.Format(_T("%04d%c%02d%c%02d"), nYear, m_cSep, nMonth, m_cSep, nDay);
