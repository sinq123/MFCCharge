...
LRESULT CCheckComboBox::OnCtlColorListBox(WPARAM wParam, LPARAM lParam) 
{
	...
	if (m_hListBox == 0) {
		...
		if (hWnd != 0 && hWnd != m_hWnd) {
			...
			m_hListBox = hWnd;
			// Do the subclassing
			//#define GWL_WNDPROC         (-4) GWL_WNDPROC
...
void CCheckComboBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	// Otherwise it is one of the items
		...
		ComboItemData* ItemData = (ComboItemData*)GetItemData(lpDrawItemStruct->itemID);
		BOOL bCheck = ItemData->m_bCheck;
		nCheck = 1 + ( bCheck != 0);
		TEXTMETRIC metrics;
...
LRESULT CCheckComboBox::OnGetText(WPARAM wParam, LPARAM lParam)
{
	...
	//LPTSTR lpString1;
    //LPCTSTR lpString2;
	//lstrcpyn(lpString1, m_strText, (INT)wParam);
...
void CCheckComboBox::RecalcText()
{
	if (!m_bTextUpdated) {
		...
		for (INT i = 0; i < nCount; i++)
		{
			ComboItemData* ItemData = (ComboItemData*)GetItemData(i);
			if (ItemData->m_bCheck) 
			{
				CString strItem;
				GetLBText(i, strItem);
				if (!strText.IsEmpty())
...
{
	//INT nResult = SetItemData(nIndex, bFlag); //lizhe
	ComboItemData* ItemData = NULL;
	ItemData = (ComboItemData*)GetItemData(nIndex);
	if(NULL == ItemData)
	{
		ItemData = new ComboItemData();
	}
	ItemData->m_bCheck = bFlag;
	INT nResult = SetItemData(nIndex, (DWORD_PTR)ItemData); //lizhe
	if (nResult < 0)
		return nResult;
...
{
	ComboItemData* ItemData = (ComboItemData*)GetItemData(nIndex);
	if(ItemData == NULL)
	{
		return FALSE;
	}
	else
	{
		try
		{
			BOOL bItemCheck = ItemData->m_bCheck;
			return bItemCheck;
		}
		catch(char* str)
		{
...
// Set Item Data
INT CCheckComboBox::SetComboItemData(INT nIndex,INT nDocClsId,BOOL bCheck)
{
	ComboItemData* ItemData = new ComboItemData();
	ItemData->m_DocClassId = nDocClsId;
	ItemData->m_bCheck = bCheck;
	INT nResult = SetItemData(nIndex, (DWORD_PTR)ItemData); //lizhe
	if (nResult < 0)
		return nResult;
	return 1;
...
// Get Item Data
INT CCheckComboBox::GetComboItemData(INT nIndex)
{
	DWORD_PTR dwpItemData = GetItemData(nIndex);
	ComboItemData* ItemData = (ComboItemData*)dwpItemData;
	if(ItemData == NULL)
	{
		return 0;
	}
