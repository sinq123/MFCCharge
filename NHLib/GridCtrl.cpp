...
//                                Fixec typo (my fault!) in CMemDCEx.h - Claus Arend-Schneider)
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	if (bOSVI)
	{
		...
		if(osvi.dwMajorVersion <= 5 /*&& && osvi.dwMinorVersion == 1*/)//5000:2000,5001:xp,5002:win2003  
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    for (row = minVisibleRow; row <= maxVisibleRow; row++)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
           ...
           DrawCell(pDC, row, col, rect);//this is needed anytime
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    ...
    if (pDC)
    {
        ...
        if (nRow < m_nFixedRows || nCol < m_nFixedCols)
        {
			...
			{
	 	        pTopLeftCell=GetCell(pCell->m_TopleftRow,pCell->m_TopleftCol);

               
				nRow=pCell->m_TopleftRow;
	            nCol=pCell->m_TopleftCol;
        ...
        else
        {
			...
			//the GetCellRangeRect(),so i manpower adjust the "rect" variable
            // CCellRange  CombineRange; 
			 CGridCell   *pCell=GetCell(nRow,nCol);
			 CGridCell   *pTopLeftCell;
			 CRect rectTemp;
             int theRightCol=0;
			...
			
			{
				pTopLeftCell=GetCell(pCell->m_TopleftRow,pCell->m_TopleftCol);
				   ...
				   //CombineRange.Set(pCell->m_TopleftRow,pCell->m_TopleftCol,\
					  // pCell->m_TopleftRow+pTopLeftCell->m_numHhide,\
					  // pCell->m_TopleftCol+pTopLeftCell->m_numVhide);
				   //GetCellRangeRect(CombineRange,rect);
				   nRow=pCell->m_TopleftRow;
				   nCol=pCell->m_TopleftCol;
				   theRightCol=nCol+pTopLeftCell->m_numVhide;
			       theBottomRow=nRow+pTopLeftCell->m_numHhide;
			} 
			else if(pCell->m_numHhide!=0||pCell->m_numVhide!=0)
			{
                  //CombineRange.Set(nRow,nCol,nRow+pCell->m_numHhide,\
					 // nCol+pCell->m_numVhide);
				  //GetCellRangeRect(CombineRange,rect);
                  theRightCol=nCol+pCell->m_numVhide;
				  theBottomRow=nRow+pCell->m_numHhide;
			}
               


				if(pCell->m_bHide==TRUE||pCell->m_numHhide!=0||pCell->m_numVhide!=0)
				{
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject)
{
    ...
    CString strText = (LPCWSTR)(LPCSTR)szBuffer;
    delete szBuffer;
    ...
    do {
        ...
        LPTSTR pszCellText = wcstok_s(szLine, _T("\t,\n"), NULL);
        while (pszCellText != NULL)
        {
			CGridCell *pCell;//added for combine
            CCellID TargetCell(cell.row + nLine, cell.col + nColumn);
			pCell=GetCell(cell.row + nLine, cell.col + nColumn);//added for combine
            if (IsValid(TargetCell)&&!pCell->m_bNoEdit)//added (&&!pCell->m_bNoEdit)for combine
            {
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point) const
{
	...
	CGridCell *pCell,*pTheTopLeft;
	pCell=GetCell(idCurrentCell.row,idCurrentCell.col);
	

 if(!(IsValid(idCurrentCell.row,idCurrentCell.col)))//the source code existing bug,adjust it!
		return FALSE;
 if(pCell->m_bHide==TRUE)
	{
		pTheTopLeft=GetCell(pCell->m_TopleftRow,pCell->m_TopleftCol);
		if(pTheTopLeft->m_numHhide==0)idCurrentCell.col=0;
		if(pTheTopLeft->m_numVhide==0)idCurrentCell.row=0;
	}
        ...
        || (m_MouseMode == MOUSE_NOTHING && (nFlags & MK_LBUTTON)))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING && m_bTitleTips)
        {
			...
			if(!IsValid(idCurrentCell ))
				return;
			CGridCell *pCell,*pTopLeftCell=NULL;
			CCellRange theTopLeftCell;
			CRect rectTemp;
			int theBottomRow=0;
			int  theRightCol=0;
			pCell=GetCell(idCurrentCell.row,idCurrentCell.col);
			 ...
			 if(pCell->m_bHide==TRUE)
			  {
                  ...
                  theTopLeftCell.Set(pCell->m_TopleftRow,pCell->m_TopleftCol,\
			      pCell->m_TopleftRow+pTopLeftCell->m_numHhide,\
			      pCell->m_TopleftCol+pTopLeftCell->m_numVhide);
				  theBottomRow=pCell->m_TopleftRow+pTopLeftCell->m_numHhide;
				  theRightCol=pCell->m_TopleftCol+pTopLeftCell->m_numVhide;
			  }

		if(pCell->m_bHide==TRUE||pCell->m_numHhide!=0||pCell->m_numVhide!=0)
		{
		  if(GetCellRect(theBottomRow,theRightCol,rectTemp)&&GetCellRangeRect(theTopLeftCell, CellRect))
		  {
			 if( CellRect.right!=rectTemp.right)
				CellRect.right=rectTemp.right;
			 if( CellRect.bottom!=rectTemp.bottom)
				CellRect.bottom=rectTemp.bottom;
           ...
           else 
			{
			  ...
			  m_TitleTip.Show(CellRect, 
                                GetItemText(pCell->m_TopleftRow,pCell->m_TopleftCol), 
                                0,
                                CellRect,
                                GetItemFont(pCell->m_TopleftRow,pCell->m_TopleftCol));
...
BOOL CGridCtrl::SetPrintStatTime(LPCTSTR strStatTime)
{
	m_strStatTime = strStatTime;
	return TRUE;
}
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    int nFontSize = -9;
    CString strFontName = L"Times New Roman";
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
	...
	// print Stat time on top left margin
    pDC->DrawText(m_strStatTime, &rc, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER);
...
BOOL CGridCtrl::DrawFixedCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
   ...
   //want remove it?
			...
			
	{
		...
		if( pCell->m_numHhide!=0||pCell->m_numVhide!=0)//扩充左上角到合适的位置
			   {  
				   
				   /*CombineRange.Set(nRow,nCol,nRow+pCell->m_numHhide,nCol+pCell->m_numVhide);
				   GetCellRangeRect(CombineRange,rect);*/
	...
	if(pCell->m_bHide==TRUE||pCell->m_numHhide!=0||pCell->m_numVhide!=0)
	{
		...
		if(GetCellRect(theBottomRow,theRightCol,rectTemp))
		{
			...
			int minVisibleRow = idTopLeft.row,\
		     ...
		     minVisibleRow=minVisibleRow>nRow?minVisibleRow:nRow;
		     minVisibleCol=minVisibleCol>nCol?minVisibleCol:nCol;
			if( theRightCol >GetFixedColumnCount())
			{ 
				
               
               GetCellRect(nRow,minVisibleCol,rectTemp);
			
			    rect.left=rectTemp.left;
			
				rect.top=rectTemp.top;
			}
            else if( theBottomRow >GetFixedRowCount())
			{
				
				
               GetCellRect(minVisibleRow,nCol,rectTemp);
			...
			{
               GetCellRect(nRow,nCol,rectTemp);
    ...
    if (!GetItem(&Item))
        return FALSE;
    if(pCell->m_bHide==TRUE||pCell->m_numHhide!=0||pCell->m_numVhide!=0)
		if( !(theBottomRow >GetFixedRowCount()))
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
	...
	CGridCell *pCell;
	pCell=GetCell(nRow,nCol);
	CCellRange theTopLeft;
     CGridCell *ptheTopLeft;
	 CRect rectCombine,rectTemp;

	 int theRightCol=0;
	 int theBottomRow=0;
	if(pCell->m_numHhide!=0||pCell->m_numVhide!=0)
	{
      
       
	  theTopLeft.Set(nRow,nCol,pCell->m_numHhide+nRow,pCell->m_numVhide+nCol);
	  
	  GetCellRangeRect(theTopLeft,rectCombine);
	  rect=rectCombine;

      theBottomRow=pCell->m_numHhide+nRow;
       theRightCol=pCell->m_numVhide+nCol;
	}
	if(pCell->m_bHide==TRUE)//如果被隐藏，显然应当编辑最左上角
	{
       
	   
       ptheTopLeft=GetCell(pCell->m_TopleftRow,pCell->m_TopleftCol);
       theTopLeft.Set(pCell->m_TopleftRow,pCell->m_TopleftCol,\
		ptheTopLeft->m_numHhide+pCell->m_TopleftRow,ptheTopLeft->m_numVhide+pCell->m_TopleftCol);
       ...
       theBottomRow=nRow+ptheTopLeft->m_numHhide;
       theRightCol=nCol+ptheTopLeft->m_numVhide;
	 }
...
{
    if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols) return L"";
...
CGridCell* CGridCtrl::GetCellP(int nRow, int nCol)
{
...
void CGridCtrl::SetCellCombine(int nRow, int nCol, int xNum, int yNum,BOOL bNoEdit)//设置单元格的合并特性
//nRow ,nCol  分别为欲合并的几个单元格的TOPLEFT位置,xNum为其紧临的水平方向的单元格数目
...
//欢迎使用和改进。
{
	xNum=abs(xNum);
	yNum=abs(yNum);
	if(!(IsValid(nRow,nCol)&&IsValid(nRow+xNum,nCol+yNum)))
		return;

	if(IsCellFixed(nRow,nCol))return;
	...
	{
		pCell->m_bSkipV=TRUE;
		pCell->m_numVhide=yNum;
	}
	pCell->m_bNoEdit=bNoEdit;
	...
	for ( i=0;i<=xNum;i++)
		...
		for( j=0;j<=yNum;j++)
		{
			...
			pCell=GetCell(nRow+i,nCol+j);
            pCell->m_bHide=TRUE;
			pCell->m_TopleftCol=nCol;
			pCell->m_TopleftRow=nRow;
			pCell->m_bNoEdit=bNoEdit;
			if(j<yNum)
				pCell->m_bSkipV=TRUE;
			if(i<xNum)
...
void CGridCtrl::UndoCellCombine(int nRow, int nCol)//左上角的单元格座标
{

  if(!IsValid(nRow,nCol))
	return;
  CGridCell *pcellold,*pCell;
  pcellold=GetCell(nRow,nCol);
  if(pcellold->m_numHhide==0&&pcellold->m_numVhide==0)
	  return;

	for (int i=0;i<=pcellold->m_numHhide;i++)
	
	
		for(int j=0;j<=pcellold->m_numVhide;j++)
		{
			if(i==0&&j==0) continue;
			pCell=NULL;
			pCell=GetCell(nRow+i,nCol+j);
            pCell->m_bHide=FALSE;
			pCell->m_TopleftCol=0;
			pCell->m_TopleftRow=0;
			pCell->m_bNoEdit=FALSE;
			if(j<pcellold->m_numVhide)
				pCell->m_bSkipV=FALSE;
			if(i<pcellold->m_numHhide)
				pCell->m_bSkipH=FALSE;

		}
		pcellold->m_bSkipH=pcellold->m_bSkipV=pcellold->m_bNoEdit=FALSE;
		pcellold->m_numHhide=pcellold->m_numVhide=0;


}

void CGridCtrl::SetFixedCellCombine(int nRow, int nCol, int xNum, int yNum)

{
	...
	if(!IsCellFixed(nRow+xNum,nCol+yNum))return;
