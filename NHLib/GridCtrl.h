...
class AFX_EXT_CLASS CGridCell : public CObject
{
public:
	BOOL m_bSkipV,m_bSkipH;//前一个表明去掉单元格的垂直线段,后一个表明去掉单元格的水平线段
    BOOL m_bHide;//单元格是否被合并(需要隐藏)
	BOOL m_bNoEdit;
	int m_numHhide;//水平需要隐藏的单元格;只针对TOPLEFT赋值.即列数
	int m_numVhide;//垂直需要隐藏的单元格;只针对TOPLEFT赋值,即行数
	int m_TopleftRow,m_TopleftCol;//被隐藏的单元格需要输入最左上的行和列值
...
public:
    ...
    void Print(); 
	BOOL SetPrintStatTime(LPCTSTR strStatTime);
#endif
#ifndef _WIN32_WCE
...
public:
	...
	void UndoCellCombine(int nRow, int nCol);
	void SetCellCombine(int nRow,int nCol,int  xNum,  int  yNum,BOOL bNoEdit=FALSE);
...
protected:
	...
	CString     m_strStatTime;          // Stat begintime to endtime.
