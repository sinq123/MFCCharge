...
CSxGroupBox::CSxGroupBox()
{
	...
	m_txtFont = new CFont;
	m_txtFont->CreateFontIndirect(&m_lf);
...
void CSxGroupBox::SetFontSize(const int nSize)
{
	m_lf.lfHeight = nSize;
...
void CSxGroupBox::SetFontBold(const bool bBold)
{
	if (bBold)
	{
...
void CSxGroupBox::SetFontName(const CString& strFontName)
{
...
{
	assert(NULL != lpLogFont);

	CopyMemory(&m_lf, lpLogFont, sizeof(m_lf));
